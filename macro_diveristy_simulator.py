...
def run_simulation(sim_config_dict):
    ...
    for slot in range(sim_duration):
        # For each transmission, regenerate new coordinates for all nodes
        # coordinates_devices_array, coordinates_bs_array = deploy_nodes(width, device_nb, bs_nb, coord_type)
        #
        # device_base_table, path_loss_matrix = \
        #     nodes_location_process(device_nb, bs_nb, coordinates_devices_array, coordinates_bs_array, path_loss, coord_type)
        ...
        trans_power_levels = np.tile(np.array([LM[int(k)] for k in sim_history[slot, :, 0]]), (bs_nb, 1))
        ...
        for device_id, trans_index in enumerate(sim_history[slot, :, 0]):
            ...
            sim_history[slot, device_id, int(trans_index)] = trans_power_levels[0, device_id]
        ...
        rec_power_levels = trans_power_levels*fadings*shadowings*path_loss_matrix
        shadowings2 = np.random.lognormal(BETA*mu_shadowing, sigma, size=(bs_nb, device_nb))
        fadings2 = np.random.exponential(scale=mu_fading, size=(bs_nb, device_nb))
        # calculate for each BS the cumulative interference level => sum along each row!
        cumu_itf_levels = np.sum(rec_power_levels, axis=1)
        # The matrix of cumulative interference at each BS
        cumu_itf_matrix = np.tile(cumu_itf_levels, (device_nb, 1)).transpose() # demension of bs_nb * device_nb

        #TODO: it just need one vector instead of matrix here.
        sim_history[slot, :, 0] = np.array([1 if device_id ==0 else 0 for device_id in range(device_nb)])
        trans_power_levels = np.tile(np.array([LM[int(k)] for k in sim_history[slot, :, 0]]), (bs_nb, 1))
        rec_power_levels2 = trans_power_levels * shadowings2 * fadings2 * path_loss_matrix

        f2 = np.vectorize(f, otypes=[np.bool])
        curr_trans_matrix = f2(rec_power_levels2, cumu_itf_matrix, threshold)
        ...
        # Allocate the nearest base station, according to the min of distance of each row
            ...
            # shadowings *= np.exp(0.5*np.power(sigma/path_loss, 2))
            # # sigma_Y = sigma/path_loss
            # device_base_table = (d2bs_dist_matrix*shadowings).argmin(axis=0)
...
def sim_result_statistics(sim_config_dict, device_nb, coordinates_devices_array, sim_history):
    ...
    for stat_percent in stat_percents:
        ...
        trans_index_df = pd.DataFrame(trans_index_array)
        # trans_index_df.to_csv("xxx", mode='a', sep=';', index=False, header=False)
