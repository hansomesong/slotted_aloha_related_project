...
from fading_shadowing.plot_generation_scripts.com_plot_config_fading_shadowing import *


SINR_THLD = 3.0

SETTING_0['SINR_THLD'] = SINR_THLD
SETTING_1['SINR_THLD'] = SINR_THLD
# SETTING_3['SINR_THLD'] = SINR_THLD[0]

SETTING = [SETTING_0, SETTING_1]

# X and Y Range for capture ratio 0dB
X_RANGE = [[0.3, 0.8], [0.3, 0.8], [0.3, 0.8], [0.3, 0.8]]
Y_RANGE = [[1e-3, 1.001], [0.2, 0.7], [0, 1.0], [1, 5]]
...
for setting in SETTING:
    ...
    ANA_DATA_FOLDED = os.path.join(LOG_DIR, setting['ANA_DIR'], setting['SUB_DIR'], CASE_DIR)
    for LM_comb, color, marker, marker_every, l_type, label in zip(
        ...
        ana_result_f = os.path.join(
            ANA_DATA_FOLDED,
            # os.path.join(setting['LOG_DIR'], setting['ANA_DIR'], setting['SUB_DIR'], CASE_DIR),
