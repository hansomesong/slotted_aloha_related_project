...
__author__ = 'qsong'
'''
    This script is used to draw the plot for case where fading and shadowing (i.e., imperfect power control) is considered.
    The generated figure is composed by three subfigures, respectively for SINR = -3dB, 0dB, 3dB.
'''
...
X_RANGE_STEP = 0.1
Y_RANGE = [1e-4, 1.0]
FIG_NAME = "packet_loss_rate_ci_with_fading_shadowing.eps"
...
for case_nb in range(len(SUB_DIR_LIST)):
    for sinr_index in range(len(SINR_THLD)):
        ...
        curr_ax.errorbar(
            sim_no_result[0],
            sim_no_plr,
            yerr=[sim_no_plr_lower, sim_no_plr_upper],
            fmt='*',
            ecolor='b',
            mfc='b',
            capthick=2,
        ...
        curr_ax.errorbar(
            sim_more_result[0],
            sim_more_plr,
            yerr=[sim_more_plr_lower, sim_more_plr_upper],
            fmt='o',
            ecolor='g',
            mfc='g',
            capthick=2,
        ...
        curr_ax.errorbar(
            sim_less_result[0],
            sim_less_plr,
            yerr=[sim_less_plr_lower, sim_less_plr_upper],
            fmt='^',
            ecolor='r',
            mfc='r',
            capthick=2,
...
#TODO: Need to do simulation for SINR=3dB, red color
ordered_handles = [handles[0], handles[3], handles[1], handles[4], handles[2]]
ordered_lables = [labels[0], labels[3], labels[1], labels[4], labels[2]]
...
print "The generated figure is place under:", os.path.join(OUTPUT_FIG_DIR, FIG_NAME)
plt.show()
