...
if __name__ == '__main__':
    ...
    CURVE_LABELS = [r'Slotted ALOHA', r'Pure ALOHA, AVG.', r'Pure ALOHA, MAX.']
    ...
    for i in range(3):
        PURE, ITF_MEAN = ALOHA_TYPES[i], ITF_MEANS[i]
    ...
    for i in range(3):
        ...
        for gamma in gammas:
            ...
            sim_fit_p_f_mrc_div[label] = mrc_curve_fitting.sim_fitted_function(
    ...
    for i, gamma in enumerate(gammas):
        ...
        for j in range(3):
            ...
            if SC_SHOW:
                ...
                axes.plot(
                L,
                p_f_rx_div_group[j][label],
                color='r',  marker='', linestyle=LINES_STYLES[j], linewidth=LINEWIDTH, label=CURVE_LABEL
                )
            # plot curves for MRC
            if MRC_SHOW:
                ...
                axes.plot(
                    L,
                    p_f_mrc_div_group[j][label],
                    color='m',  marker='', linestyle=LINES_STYLES[j], linewidth=LINEWIDTH, label=CURVE_LABEL
                )
            ...
            if FIT_MRC_SHOW:
                ...
                axes.plot(
                    L,
                    fit_p_f_mrc_div_group[j][label],
                    color='b',  marker='', linestyle=LINES_STYLES[j], linewidth=LINEWIDTH, label=CURVE_LABEL
                )
            ...
            if SIM_FIT_MRC_SHOW:
                ...
                axes.plot(
                    L,
                    sim_fit_p_f_mrc_div_group[j][label],
                    color='b',  marker='', linestyle=LINES_STYLES[j], linewidth=LINEWIDTH, label=CURVE_LABEL
                )
    ...
    for i, gamma in enumerate(gammas):
        ...
        for j in range(3):
            if SIM_SC_SHOW:
                axes.errorbar(
                    sim_intensity_group[j][label],
                    sim_plr_sc_divers_group[j][label],
                    yerr=[sim_plr_sc_divers_semi_ci_group[j][label],  sim_plr_sc_divers_semi_ci_group[j][label]],
    ...
    for i, gamma in enumerate(gammas):
        ...
        for j in range(3):
            if SIM_MRC_SHOW:
                ...
                axes.errorbar(
                    sim_intensity_group[j][label],
                    sim_plr_mrc_divers_group[j][label],
                    yerr=[sim_plr_mrc_divers_semi_ci_group[j][label],  sim_plr_mrc_divers_semi_ci_group[j][label]],
