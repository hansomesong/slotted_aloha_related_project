...
S_P_IDENTIC = "ideal analytical, identical power"
S_P_INCREMENT = "ideal analytical, power increment"
S_P_DECREMENT = "ideal analytical, power decrement"
...
sim_no_intensity, sim_no_plr = sim_data_process(3, 1, 1, 1, 36)
sim_more_intensity, sim_more_plr = sim_data_process(3, 2, 1, 1, 36)
sim_less_intensity, sim_less_plr = sim_data_process(3, 1, 2, 1, 36)

ana_result_f_no = os.path.join(DATA_FOLDED, "shadowing_analytical_result_threshold=3dB_l=1_m=1_sigma=0.0001.csv")
ana_result_f_more = os.path.join(DATA_FOLDED, "shadowing_analytical_result_threshold=3dB_l=2_m=1_sigma=0.0001.csv")
ana_result_f_less = os.path.join(DATA_FOLDED, "shadowing_analytical_result_threshold=3dB_l=1_m=2_sigma=0.0001.csv")

ana_result_f_no_ideal = os.path.join(DATA_FOLDED, "analytical_result_threshold=3.0_l=1_m=1.csv")
ana_result_f_more_ideal = os.path.join(DATA_FOLDED, "analytical_result_threshold=3.0_l=2_m=1.csv")
ana_result_f_less_ideal = os.path.join(DATA_FOLDED, "analytical_result_threshold=3.0_l=1_m=2.csv")
...
ana_no_intensity_ideal, ana_no_plr_ideal = analytic_data_process(ana_result_f_no_ideal)
ana_more_intensity_ideal, ana_more_plr_ideal = analytic_data_process(ana_result_f_more_ideal)
ana_less_intensity_ideal, ana_less_plr_ideal = analytic_data_process(ana_result_f_less_ideal)
...
ax1.plot(ana_no_intensity_ideal, ana_no_plr_ideal, color='b', marker='*', linestyle="", linewidth=2, label=S_P_IDENTIC)
ax1.plot(ana_more_intensity_ideal, ana_more_plr_ideal, color='g', marker='o', linestyle="", linewidth=2, label=S_P_INCREMENT)
ax1.plot(ana_less_intensity_ideal, ana_less_plr_ideal, color='r', marker='^', linestyle="", linewidth=2, label=S_P_DECREMENT)
...
ax1.set_title("Comparison between perfect power control and imperfect power control, SINR Threshold 3dB")
ax1.grid()
...
plt.savefig('comp_between_ideal_no_ideal.eps', format='eps', dpi=300)
plt.show()
