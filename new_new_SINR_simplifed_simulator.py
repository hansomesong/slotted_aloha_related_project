...
def run_simulation(sim_config_dict):
    ...
    for slot in range(sim_duration):
        ...
        # Now generate the real rec_power_levels! Another sampling for fading and shadowing.
        ...
        f = np.vectorize(f, otypes=[np.bool])
        # curr_trans_matrix = np.apply_along_axis(calculate_sinr, 1, rec_power_levels, threshold, sim_history[slot, :, 0])
        curr_trans_matrix = f(rec_power_levels2, cumu_itf_matrix, threshold)

        # print "rec", rec_power_levels2
        # print "cumu", cumu_itf_matrix
